...
<button onclick="toggleDiscount()">50% ZNIŻKI: OFF</button><button onclick="changeBg()">🎨 Zmień tło</button></div></div><script>
let n=0;let discount=false;let bg=0;const bgs=['#111','#0a1628','#2d1b4e','#0d2818','#3d1620'];
function upd(){inp.value=n;c.textContent=n;let sum=n*15000;if(discount)sum=Math.round(sum*0.5);t.textContent=sum.toLocaleString('pl-PL')+' $';s.textContent='Wybrano: '+n+' zestawów';}
...
<script>
function toggleDiscount(){discount=!discount;document.querySelectorAll('button')[4].textContent='50% ZNIŻKI: '+(discount?'ON':'OFF');upd();}
function changeBg(){bg=(bg+1)%bgs.length;document.body.style.background=bgs[bg];}
</script>
